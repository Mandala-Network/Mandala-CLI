...
import {
  ...
  isMandalaConfig, chooseMandalaCloudURL, findConfigByNameOrIndex
} from './config.js';
import {
  ensureRegistered, safeRequest, buildAuthFetch, handleRequestError,
  printProjectList, printAdminsList, printLogs, printReleasesList
} from './utils.js';
import type {
  MandalaConfig, MandalaConfigInfo, ProjectInfo, ProjectListing,
  AdminInfo, DeployInfo, AccountingRecord,
  LogPeriod, LogLevel
} from './types.js';
...
function printDomainInstructions(projectID: string, domain: string, domainType: 'frontend' | 'backend') {
...
async function setCustomDomain(config: MandalaConfig, domainType: 'frontend' | 'backend', domain: string, interactive: boolean) {
  if (!config.projectID) {
  ...
  while (retry) {
    try {
      let result: any = await client.fetch(`${config.MandalaCloudURL}/api/v1/project/${config.projectID}/domains/${domainType}`, {
        method: 'POST',
...
async function viewAndEditWebUIConfig(config: MandalaConfig) {
  if (!config.projectID) {
...
async function viewBillingStats(config: MandalaConfig) {
  const client = await buildAuthFetch(config);
  ...
  const records = await safeRequest<{ records: AccountingRecord[] }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/billing/stats`, data);
  if (!records) return;
...
export async function showProjectInfo(config: MandalaConfig) {
  if (!config.projectID) {
    console.error(chalk.red('No project ID set.'));
    return;
  }
...
async function topUpProjectBalance(config: MandalaConfig) {
  if (!config.projectID) {
    console.error(chalk.red('No project ID set.'));
    return;
  }
...
async function deleteProject(config: MandalaConfig) {
  if (!config.projectID) {
    console.error(chalk.red('No project ID set.'));
    return;
  }
...
async function setupGitHubActionsWizard(config: MandalaConfig) {
  ...
  const configIndex = (info.configs || []).findIndex(
    c => c.name === config.name && c.provider === config.provider && c.MandalaCloudURL === config.MandalaCloudURL && c.projectID === config.projectID
  );
  if (configIndex === -1) {
  ...
  try {
    ...
    await remakeWallet(originalWalletClient as any, config.network as WalletNetwork);
  }
...
async function editAdvancedEngineConfig(config: MandalaConfig) {
  if (!config.projectID) {
    console.error(chalk.red('No project ID set.'));
    return;
  }
  ...
  const infoResp = await safeRequest<any>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/info`, {});
  if (!infoResp) return;
  let engineConfig: any = infoResp.engine_config || {};
  ...
  while (!done) {
    ...
    if (selectedTopic === 'back') {
      ...
      if (action === 'remove') {
        ...
        const splitted = endpoints.split(',').map((e: string) => e.trim()).filter((x: string) => !!x);
...
async function triggerAdminEndpoint(config: MandalaConfig, endpoint: 'syncAdvertisements' | 'startGASPSync' | 'evictOutpoint', txid?: string, outputIndex?: string, service?: string) {
  if (!config.projectID) {
    console.error(chalk.red('No project ID set.'));
    return;
  }
...
async function promptResourceLogParameters(): Promise<{ resource: string; since: LogPeriod; tail: number; level: LogLevel }> {
  ...
  const { tail } = await inquirer.prompt([
    { type: 'number', name: 'tail', message: 'Number of lines to tail (1-10000):', default: 1000, validate: (val: number) => val > 0 && val <= MAX_TAIL_LINES ? true : 'Invalid tail number' }
  ]);
...
export async function fetchResourceLogs(config: MandalaConfig, params?: { resource?: string; since?: string; tail?: number; level?: string }) {
  if (!config.projectID) {
...
export async function pickReleaseId(config: MandalaConfig, providedReleaseId?: string): Promise<string | undefined> {
  if (providedReleaseId) return providedReleaseId;
  const client = await buildAuthFetch(config);
  const result = await safeRequest<{ deploys: DeployInfo[] }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/deploys/list`, {});
...
export async function projectMenu() {
  ...
  while (!done) {
    ...
    if (action === 'ls') {
      ...
      if (!config.projectID) { console.error(chalk.red('No project ID.')); continue; }
      const client = await buildAuthFetch(config);
      const result = await safeRequest<{ admins: AdminInfo[] }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/admins/list`, {});
      if (result) {
      ...
      if (!config.projectID) { console.error(chalk.red('No project ID.')); continue; }
      const client = await buildAuthFetch(config);
      const result = await safeRequest<{ logs: string }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/logs/project`, {});
      if (result && typeof result.logs === 'string') {
