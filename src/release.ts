...
export async function releaseMenu() {
  ...
  while (!done) {
    ...
    if (action === 'get-upload-url') {
      ...
      const result = await safeRequest<{ url: string, deploymentId: string }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/deploy`, {});
      ...
      const client = await buildAuthFetch(config);
      const result = await safeRequest<{ logs: string }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/logs/deployment/${releaseId}`, {});
      ...
      const client = await buildAuthFetch(config);
      const result = await safeRequest<{ logs: string }>(client, config.MandalaCloudURL, `/api/v1/project/${config.projectID}/logs/deployment/${deploymentId}`, {});
      if (result && typeof result.logs === 'string') {
        printLogs(result.logs, 'Release Logs');
      }
    } else if (action === 'now') {
