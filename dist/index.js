...
import { loadMandalaConfigInfo, saveMandalaConfigInfo, pickMandalaConfig, isMandalaConfig, addMandalaConfigInteractive, editMandalaConfigInteractive, deleteMandalaConfig, findConfigByNameOrIndex, chooseMandalaCloudURL, printAllConfigsWithIndex, configMenu } from './config.js';
...
import { ensureRegistered, safeRequest, buildAuthFetch, handleRequestError, uploadArtifact, printProjectList, printAdminsList, printLogs, printReleasesList } from './utils.js';
program
    .name('mandala')
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/admins/list`, {});
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/logs/project`, {});
    if (result)
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/deploys/list`, {});
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    try {
        let result = await client.fetch(`${cfg.MandalaCloudURL}/api/v1/project/${cfg.projectID}/domains/frontend`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    try {
        let result = await client.fetch(`${cfg.MandalaCloudURL}/api/v1/project/${cfg.projectID}/domains/backend`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ domain })
        });
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const projectInfo = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/info`, {});
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const records = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/billing/stats`, data);
    if (!records)
        return;
...
projectCommand
    ...
    if (!amount || amount <= 0) {
        const answers = await inquirer.prompt([
            { type: 'number', name: 'amount', message: 'Enter amount in satoshis:', validate: (val) => val > 0 ? true : 'Amount must be positive.' }
        ]);
...
releaseCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/deploy`, {});
...
releaseCommand
    ...
    if (!finalReleaseId)
        return;
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.MandalaCloudURL, `/api/v1/project/${cfg.projectID}/logs/deployment/${finalReleaseId}`, {});
    if (result)
