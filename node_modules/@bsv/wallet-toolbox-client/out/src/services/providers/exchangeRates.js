...
async function updateExchangeratesapi(targetCurrencies, options) {
    ...
    if (!usdPerBase || typeof usdPerBase !== 'number') {
        throw new WERR_errors_1.WERR_BAD_REQUEST(`getExchangeRatesIo missing rate for 'USD'`);
    }
    const r = {
    ...
    for (const currency of targetCurrencies) {
        ...
        if (!curPerBase || typeof curPerBase !== 'number') {
            throw new WERR_errors_1.WERR_BAD_REQUEST(`getExchangeRatesIo missing rate for '${currency}'`);
        }
        r.rates[currency] = curPerBase / usdPerBase;
    }
    return r;
}
async function getExchangeRatesIo(key, symbols) {
