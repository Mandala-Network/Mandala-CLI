...
class Wallet {
    constructor(argsOrSigner, services, monitor, privilegedKeyManager, makeLogger) {
        /**
    ...
    validateAuthAndArgs(args, validate, logger) {
    ...
    async listOutputs(args, originator) {
        ...
        if (r.BEEF) {
            this.beef.mergeBeefFromParty(this.storageParty, (0, utilityHelpers_noBuffer_1.asArray)(r.BEEF));
    ...
    async listCertificates(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        const { vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateListCertificatesArgs);
    ...
    async acquireCertificate(args, originator) {
        ...
        if (args.acquisitionProtocol === 'direct') {
            const { auth, vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateAcquireDirectCertificateArgs);
        ...
        if (args.acquisitionProtocol === 'issuance') {
            const { auth, vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateAcquireIssuanceCertificateArgs);
    ...
    async relinquishCertificate(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        this.validateAuthAndArgs(args, sdk_1.Validation.validateRelinquishCertificateArgs);
        const r = await this.storage.relinquishCertificate(args);
        return { relinquished: true };
    }
    async proveCertificate(args, originator) {
        originator = sdk_1.Validation.validateOriginator(originator);
        const { auth, vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateProveCertificateArgs);
        const r = await (0, proveCertificate_1.proveCertificate)(this, auth, vargs);
        return r;
    }
    async discoverByIdentityKey(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        this.validateAuthAndArgs(args, sdk_1.Validation.validateDiscoverByIdentityKeyArgs);
        const TTL_MS = 2 * 60 * 1000;
        const now = Date.now();
        // --- trustSettings cache (2 minutes) ---
        let trustSettings = this._trustSettingsCache && this._trustSettingsCache.expiresAt > now
        ...
        if (!cached || cached.expiresAt <= now) {
            const value = await (0, identityUtils_1.queryOverlay)({ identityKey: args.identityKey, certifiers }, this.lookupResolver);
        ...
        return (0, identityUtils_1.transformVerifiableCertificatesWithTrust)(trustSettings, cached.value);
    }
    async discoverByAttributes(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        this.validateAuthAndArgs(args, sdk_1.Validation.validateDiscoverByAttributesArgs);
        const TTL_MS = 2 * 60 * 1000;
        const now = Date.now();
        // --- trustSettings cache (2 minutes) ---
        let trustSettings = this._trustSettingsCache && this._trustSettingsCache.expiresAt > now
        ...
        let attributesKey = args.attributes;
        if (args.attributes && typeof args.attributes === 'object') {
            const keys = Object.keys(args.attributes).sort();
        ...
        let cached = this._overlayCache.get(cacheKey);
        if (!cached || cached.expiresAt <= now) {
            const value = await (0, identityUtils_1.queryOverlay)({ attributes: args.attributes, certifiers }, this.lookupResolver);
            cached = { value, expiresAt: now + TTL_MS };
            this._overlayCache.set(cacheKey, cached);
        }
        if (!cached.value) {
    ...
    logMakeLogger(method, args) {
        var _a;
        const logger = (_a = this.makeLogger) === null || _a === void 0 ? void 0 : _a.call(this, args['log']);
        this.logMethodStart(method, logger);
        return logger;
    }
    logMethodStart(method, logger) {
        logger === null || logger === void 0 ? void 0 : logger.group(`Wallet ${method}`);
    }
    logResult(r, logger) {
        var _a;
        if (!logger)
            return;
        logger.groupEnd();
        r['log'] = (_a = logger.flush) === null || _a === void 0 ? void 0 : _a.call(logger);
    }
    logWalletError(eu, logger) {
        var _a;
        if (!logger)
            return;
        logger.error('WalletError:', WalletError_1.WalletError.unknownToJson(eu));
        (_a = logger.flush) === null || _a === void 0 ? void 0 : _a.call(logger);
    }
    //////////////////
    // Actions
    //////////////////
    async createAction(args, originator) {
        ...
        try {
            ...
            logger === null || logger === void 0 ? void 0 : logger.log('validated args');
            ...
            logger === null || logger === void 0 ? void 0 : logger.log('action created');
            if (r.tx) {
                this.beef.mergeBeefFromParty(this.storageParty, (0, utilityHelpers_noBuffer_1.asArray)(r.tx));
            }
            if (r.tx) {
                r.tx = this.verifyReturnedTxidOnlyAtomicBEEF(r.tx, (_a = args.options) === null || _a === void 0 ? void 0 : _a.knownTxids);
                logger === null || logger === void 0 ? void 0 : logger.log('verify returned AtomicBEEF');
            }
            if (!vargs.isDelayed)
    ...
    async signAction(args, originator) {
        var _a;
        sdk_1.Validation.validateOriginator(originator);
        const { auth, vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateSignActionArgs);
    ...
    async internalizeAction(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        const { auth, vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateInternalizeActionArgs);
    ...
    async abortAction(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        const { auth } = this.validateAuthAndArgs(args, sdk_1.Validation.validateAbortActionArgs);
        const r = await this.storage.abortAction(args);
        return r;
    }
    async relinquishOutput(args, originator) {
        sdk_1.Validation.validateOriginator(originator);
        const { vargs } = this.validateAuthAndArgs(args, sdk_1.Validation.validateRelinquishOutputArgs);
     ...
     */
    async balance(args) {
        args || (args = { basket: types_1.specOpWalletBalance });
        if (args.basket !== types_1.specOpWalletBalance) {
            args.tags = [...(args.tags || []), types_1.specOpWalletBalance];
        }
...
function validateListOutputsArgs(args) {
    const vargs = sdk_1.Validation.validateListOutputsArgs(args);
    const balancePrefix = 'balance ';
    if (vargs.basket.startsWith(balancePrefix)) {
        vargs.basket = vargs.basket.slice(balancePrefix.length);
        vargs.tags = [...vargs.tags, types_1.specOpWalletBalance];
    }
    return vargs;
}
